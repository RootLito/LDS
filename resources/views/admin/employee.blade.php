...
@section('content')
    <div class="w-full h-full overflow-y-auto p-10">
        ...
        <form method="GET" class="bg-white p-4 py-6 rounded mb-6 flex flex-wrap gap-4">
            ...
            <button class="bg-blue-500 hover:bg-blue-600 text-white px-5 py-2 rounded cursor-pointer text-sm">
                <i class="fa-solid fa-sliders me-2"></i> Apply Filters
            </button>
            @if (request()->anyFilled(['name', 'status', 'office', 'position']))
                <a href="{{ url()->current() }}"
                    class="bg-gray-200 border border-gray-300 hover:bg-gray-300 text-gray-600 px-5 py-2 rounded text-sm flex items-center">
                    ...
                    @forelse($employees as $employee)
                        <tr class="{{ $loop->last ? '' : 'border-b border-gray-200' }} hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @endphp
                                <span
                                    class="px-2 py-1 text-xs font-bold rounded-full {{ $colors[$employee->status] ?? 'bg-gray-100' }}">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <button
                                    @click="openDetails = true; employee = {{ $employee->load('trainingsAttended')->toJson() }}"
                ...
                <div @click.away="openDetails = false" class="bg-white rounded shadow-2xl w-full max-w-5xl p-8 relative">
                    <div class="text-center pb-4 mb-6">
                        ...
                        <p class="text-xl font-semibold text-gray-600 mt-2">
                            <span class="inline-block px-4 py-2 font-bold text-gray-600 bg-gray-200 rounded-full"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 text-sm">
                        <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-3">
                            <div class="flex justify-between">
                                <span class="font-semibold text-gray-600"><i class="fa-solid fa-user mr-2"></i>
                                    Gender</span>
                                <span x-text="employee?.gender"></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold text-gray-600"><i class="fa-solid fa-birthday-cake mr-2"></i>
                                    Birthdate</span>
                                <span x-text="employee?.birthdate"></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold text-gray-600"><i class="fa-solid fa-user-tag mr-2"></i>
                                    Status</span>
                                <span x-text="employee?.status"></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold text-gray-600"><i class="fa-solid fa-building mr-2"></i>
                                    Office</span>
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-3">
                            <div class="flex justify-between">
                                <span class="font-semibold text-gray-600"><i class="fa-solid fa-briefcase mr-2"></i>
                                    Designation</span>
                                <span x-text="employee?.designation"></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold text-gray-600"><i class="fa-solid fa-calendar-day mr-2"></i>
                                    Appointment Date</span>
                                <span x-text="employee?.appointment_date"></span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold text-gray-600"><i class="fa-solid fa-certificate mr-2"></i>
                                    Trainings Attended</span>
                    ...
                    <div class="mt-6">
                        ...
                        <div x-show="!employee?.trainings_attended?.length" class="text-gray-500 py-6 text-center">No
                            training attended yet.</div>
                        <div x-show="employee?.trainings_attended?.length" class="overflow-y-auto"

...
@section('content')
    <div class="p-10" x-data="certificateModal()" @keydown.escape.window="isOpen = false">
        ...
        <form method="GET" action="{{ route('admin.certificates') }}"
            class="bg-white p-4 py-6 rounded mb-6 flex flex-wrap gap-4">
            <input type="text" name="name" value="{{ request('name') }}" placeholder="Search Employee Name"
                class="border border-gray-300 px-3 py-2 rounded w-100 focus:ring-2 focus:ring-blue-500 outline-none">

            <input type="text" name="title" value="{{ request('title') }}" placeholder="Search Training Title"
                class="border border-gray-300 px-3 py-2 rounded w-100 focus:ring-2 focus:ring-blue-500 outline-none">

            <div class="flex gap-2">
                <button type="submit"
                    class="bg-blue-500 hover:bg-blue-600 text-white px-5 py-2 rounded cursor-pointer text-sm">
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y">
                    @forelse($employees as $employee)
                        @php
                            $certificates = $employee->trainingsAttended
                                ->filter(fn($t) => $t->certificate_path)
                                ->values();
                        @endphp
                        <tr class="{{ $loop->last ? '' : 'border-b border-gray-200' }} hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @else
                                    <button
                                        @click="openModal({{ $certificates->map(
                                                fn($c) => [
        ...
        <div x-show="isOpen" x-transition.opacity
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-10" style="display: none;"
            @click.away="isOpen = false">
            <div class="bg-white rounded-lg overflow-hidden w-full max-w-4xl relative flex flex-col"
                ...
                <div class="flex-1 flex items-center justify-center bg-gray-100 p-4 max-h-[70vh]">
                    <img :src="modalImage" alt="Certificate" class="max-h-full max-w-full object-contain" />
                </div>

                <!-- Thumbnails Carousel -->
                <div
                    class="border-t border-gray-200 bg-white p-4 flex items-center justify-center space-x-2 overflow-x-auto scrollbar-hide no-scrollbar">

                    <template x-for="cert in employeeCertificates" :key="cert.certificate_path">
                        <img :src="cert.certificate_path" @click="selectCertificate(cert)"
                            class="cursor-pointer border border-gray-300 rounded transition w-24 h-16 object-cover flex-shrink-0"
                            :class="cert.certificate_path === modalImage ? 'opacity-100 ring-2 ring-indigo-600' :
